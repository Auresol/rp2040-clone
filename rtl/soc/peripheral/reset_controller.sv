...
module reset_controller (
    ...
    // Reset outputs
    output wire        cpu_rst_n,         // CPU reset (active low)
...
localparam [1:0] ADDR_RESET      = 2'd0,  // 0x00
                 ADDR_RESET_DONE = 2'd1,  // 0x04
                 ADDR_REASON     = 2'd2,  // 0x08
                 ADDR_CHIP_RESET = 2'd3;  // 0x0C
...
// CPU reset: asserted on POR, watchdog, or software chip reset.
// Held until firmware explicitly does something (not auto-released).
// For simplicity: CPU resets whenever any chip-wide reset occurs.
reg cpu_rst_r;
always @(posedge clk or negedge por_n) begin
    if (!por_n)
        cpu_rst_r <= 1'b1;
        ...
        cpu_rst_r <= 1'b0;  // CPU auto-releases after 1 cycle
end
...
assign hrdata = (reg_addr_r == ADDR_RESET)      ? {24'h0, reset_reg}   :
                (reg_addr_r == ADDR_RESET_DONE)  ? {24'h0, ~reset_reg}  :  // TODO: real ready handshake
